...
export default {
  data () {
    return {
      ...
      columns: [
        ...
        //               if (this.data[j].id + '' === scope.id + '') {
        //                 this.dialog.visible = true
        //                 this.addBody(this.data[j],this.data[j].title)
        //               }
        //             }
        //           }
        //         }
  ...
  created () {
    ...
    this.defaultOption = this.userInfo.username
    this.search.membername = this.defaultOption
    this.$bus.$off('my/page')
  ...
  methods: {
    ...
    async addBody (message) {
      let { classtype, isdefault, language, linkname, title } = this.data[0]
      let res = await editPage({ pageinfo: { message, classtype, isdefault, language, linkname, title } })
      if (!res.Error) {
        this.$message({
          message: res.Info,
          type: 'success'
...
<style lang="less" scoped>
  ...
  /deep/#m-table[data-v-14bc5cde] tbody .el-input__inner{
    text-align: center;
  }
</style>
