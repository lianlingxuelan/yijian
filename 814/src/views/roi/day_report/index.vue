<template>
    <d2-container class="day_report">
        ...
        <d2-crud :columns="columns" :data="YesterdayReports" :options="options"></d2-crud>
    </d2-container>
</template>
<script>
...
export default {
  ...
  methods: {
    async getDayReport () {
      let data = await roiApi.getDayReport(this.SearchContent)
      this.Months = data.Days
      this.SearchContent.Month = data.Day
      this.SearchContent.Money = data.Money
      this.YesterdayReports = data.YesterdayReports
      this.dayReports = data.YesterdayReports
      ...
      this.GroupnameList = this.YesterdayReports.reverse().filter((item, index) => {
        if (item.groupname) {
          const _index = this.YesterdayReports.findIndex((_item) => {
      ...
      this.YesterdayReports.map((item) => {
      ...
      // 合计插入第一行
      this.YesterdayReports.unshift(this.amount)
    },
    calc_spendRMB (item) {
    ...
    filter_group () {
      if (this.SearchContent.MemberName === '不过滤') {
        this.YesterdayReports = this.dayReports
      } else {
        this.YesterdayReports = this.dayReports.filter((item) => {
...
<style scoped>
    .day_report /deep/.el-table .primary-row {
